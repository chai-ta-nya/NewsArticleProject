...
const All = () => {
  ...
  const [f, setF] = useState(false); // 🔁 Toggle to re-fetch
  ...
  }, [f]); // 🔁 Runs on mount & whenever `f` changes
    ...
    <div className="cards-container">
      {posts.length > 0 ? posts.map(post => (
        <div className="news-card fade-in" key={post._id}>
          ...
          {/* 👍👎 Like/Dislike buttons if logged in */}
          {state.token && (
            <div className="reaction-buttons">
              <button
  className={`like-btn ${post.likes?.includes(state._id) ? 'active-reaction' : ''}`}
...
>
              ...
              <button
  className={`dislike-btn ${post.dlikes?.includes(state._id) ? 'dactive-reaction' : ''}`}
  onClick={() => handleDislike(post._id)}
>

                👎 {post.dlikes?.length || 0}
              </button>
            </div>
          )}
        </div>
